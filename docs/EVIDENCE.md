...
- **画面ロック抑止。** 呼吸ガイドと NSDR の再生中だけ Wake Lock を取り、
  終了時に必ず解放する。
- **セッションは画面から独立している。** 呼吸・NSDR・サウンドはそれぞれ別の
  セッションとして動き、画面を移っても止まらず、3つ同時に走らせられる。
  当初は画面を離れると停止していたが、20分の NSDR 中にサウンドを調整しに行くと
  消える、という挙動になっていたため作り直した。走っているものはタブに印を出す。
- **呼吸に合わせた音量の揺らぎは、JS ではなく音声スレッドで回す。** 1呼吸ぶんの
  包絡を AudioBuffer に書き出してループ再生し、その信号を音量の AudioParam に
  加算している（内在値 1-depth + 包絡×depth なので、倍率は 1-depth 〜 1 を往復し、
  depth の上限を 0.9 に抑えているため谷でも無音にならない）。
  毎フレーム JS から書き換える作りにしなかったのは、requestAnimationFrame が
  ブラウザを背面に回すと止まり、画面を見ていない間に音量が中途半端な値で
  固まってしまうため。サウンドは画面を見ずに使うものなので、ここは音声スレッド
  だけで完結している必要がある。
- **読み上げ中は合図音を控える。** NSDR のナレーションが喋っている間だけ、
  呼吸ガイドの合図音を 0.3 倍に落とす。併走させたときに言葉が埋もれないようにする
  ためで、サウンドのノイズ類には手を触れない（マスキングが目的なので下げると
  意味がなくなる）。
